...
if args.load_checkpoint:
    assert os.path.exists(
        f"assets/checkpoints/{args.run_id}"
...
DATA_QQP_TRAIN = pd.read_csv(
    "./assets/datasets/QQP/train.tsv", sep="\t", dtype=types_dict
)
DATA_QQP_TEST = pd.read_csv(
    "./assets/datasets/QQP/test.tsv", sep="\t", dtype=types_dict
)
...
multitask_net = MultiTaskNet(seq_pair_classifier, device=device).to(device)
...
n_batches = {
    "SST2": math.floor(len(DATA_SST_TRAIN) / 16),
    "QQP": math.floor(len(DATA_QQP_TRAIN) / 16),
}
get_batch_function = {
    "SST2": get_batch_SST2_from_indices,
    "QQP": get_batch_QQP_from_indices,
}
...
while not finished_training:
    ...
    for dataset in datasets:
        ...
        dataset_batch = get_batch_function[dataset](
            dataframe[dataset], batch_indices[dataset][idx, :]
        )
    ...
    writer.add_scalars(
        f"{run_identifier}/metrics/train",
        {datasets[i]: metrics[i] for i in range(len(datasets))},
        global_counter,
    )
