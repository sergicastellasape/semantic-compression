...
"""
class ConvAttGenerator(nn.Module):
    def __init__(self, device=torch.device('cpu')):
        self.device = device
        self.gen_net = ConvAtt(embedding_dim, device=device)


    def forward(self, inp, indices_batch, masks_dict=None, return_comp_rate=False):


    def generate(self, inp, indices_batch, masks_dict=None, return_comp_rate=False):
        ...
        for i in range(max()):
        idxs = [(1, 2), (0,)]
        for _ in range(len(idxs), b):
            idxs.append(())

        P = torch.nn.utils.rnn.pad_sequence([T[i, j_tup] for i, j_tup in enumerate(idxs)], batch_first=True)
        mask = torch.zeros(b, max([max(x, default=0) + 1 for x in idxs]))

        for i, j_tup in enumerate(idxs):
            mask[i, j_tup] = 1.
